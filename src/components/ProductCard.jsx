...
text
  ...
  <Link to={`/product/${product._id}`}>
    {/* Image */}
   <div className="w-full overflow-hidden bg-gradient-to-br from-amber-50 to-amber-100 relative">
<img
src={product.image}
alt={product.name}
className={w-full h-auto sm:h-[260px] md:h-[320px] object-cover transition duration-500 group-hover:scale-110 ${inStock ? '' : 'grayscale opacity-60'}}
...
<p className="text-gray-600 text-xs mb-3 line-clamp-2"> {isGu && product.descriptionGu ? product.descriptionGu : product.description} </p> <div className="flex items-center justify-between pt-3 border-t border-gray-200"> <div> <p className="text-xs text-gray-400">{isGu ? 'થી શરૂ' : 'From'}</p> <p className="text-xl font-black text-gray-800">₹{product.sizes?.[0]?.price || 0}</p> </div>
text
  ...
  {/* WhatsApp Button */}
  <Link to={`/product/${product._id}`}>
<a className="flex items-center justify-center gap-2 mx-4 mb-4 py-2.5 rounded-full bg-green-500 hover:bg-green-600 text-white text-sm font-bold transition-all hover:shadow-lg"> {isGu ? 'વિગતો જુઓ' : 'View Details'} </a> </Link> </div> ); };
