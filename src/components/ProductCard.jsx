...
const ProductCard = ({ product }) => {
  ...
  return (
    <div
      className={`group bg-[#D0F0C0] rounded-2xl overflow-hidden shadow-md hover:shadow-2xl transition-all duration-300 relative border ${
        inStock ? 'border-gray-100' : 'border-red-100'
      }`}
    >
      {/* Stock Badge */}
      <div
        className={`absolute top-3 left-3 z-20 flex items-center gap-1 px-2 py-1 rounded-full text-xs font-bold shadow ${
      ...
      <Link to={`/product/${product._id}`}>
        <div className="w-full overflow-hidden bg-gradient-to-br from-amber-50 to-amber-100 relative">
          <img
            src={product.image}
            alt={product.name}
            className={`w-full h-auto sm:h-[260px] md:h-[320px] object-cover transition duration-500 group-hover:scale-110 ${
      ...
      {inStock ? (
        ...
        >
          <FaWhatsapp />
          {isGu ? 'વોટ્સએપ ઓર્ડર' : 'Order on WhatsApp'}
        </a>
      ) : (
        <button
          disabled
          className="flex items-center justify-center gap-2 mx-4 mb-4 py-2.5 rounded-full bg-gray-300 text-gray-600 text-sm font-bold cursor-not-allowed"
        >
          {isGu ? 'સ્ટૉક નથી' : 'Out of Stock'}
        </button>
      )}
