    ...
    <div className="min-h-screen bg-gradient-purple overflow-x-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 sm:pt-8">
        <button
          ...
          className="flex items-center gap-2 text-orange-100 hover:text-orange-400 
                     transition font-semibold group mb-4 sm:mb-6 text-sm sm:text-base"
        >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12 sm:pb-16">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-12">
                ...
                {!imgLoaded && (
                  <div className="absolute inset-0 flex items-center justify-center z-10">
                    <div className="animate-spin rounded-full h-10 w-10 
                                    border-t-4 border-orange-400"></div>
                  </div>
                )}
                <img
                  src={displayImage}
          ...
          <div className="space-y-5 sm:space-y-6">
            ...
            {/* PRICE CARD */}
            <div className="bg-[#D0F0C0] rounded-2xl sm:rounded-3xl p-4 
                            sm:p-6 shadow-xl">
            ...
            {product.sizes?.length > 0 && (
              <div>
                <p className="font-semibold text-purple-100 mb-2 sm:mb-3 
                ...
                <div className="flex flex-wrap gap-2 sm:gap-3">
                  {product.sizes.map((sv, idx) => (
                    <button
                      ...
                      className={`px-4 sm:px-5 py-2 rounded-lg sm:rounded-xl 
                        border transition-all duration-200 text-sm sm:text-base ${
                          selectedSizeIdx === idx
            ...
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <div className="flex items-center border rounded-lg sm:rounded-xl 
                              bg-[#D0F0C0] overflow-hidden">
                <button
                  onClick={() => setQuantity(Math.max(1, quantity - 1))}
                  className="px-3 sm:px-4 py-2 text-lg font-bold 
                             hover:bg-green-200 transition"
                >
                  −
                </button>
                <span className="px-3 sm:px-4 font-semibold min-w-[2rem] 
                                 text-center">
                  {quantity}
                </span>
                <button
        ...
        <div className="mt-10 sm:mt-16">
          <div className="flex gap-1 sm:gap-2 border-b border-purple-400/30 
                          overflow-x-auto">
            {tabs.map((tab) => (
              <button
                key={tab.key}
                onClick={() => setActiveTab(tab.key)}
                className={`px-4 sm:px-6 py-2.5 sm:py-3 font-bold text-sm 
                  sm:text-base rounded-t-xl transition-all whitespace-nowrap ${
                    activeTab === tab.key
          ...
          <div className="bg-[#D0F0C0] rounded-b-2xl sm:rounded-b-3xl 
                          shadow-xl p-4 sm:p-6 md:p-8">
            
            {/* DESCRIPTION TAB */}
            {activeTab === "description" && (
              <div className="flex gap-3 sm:gap-4">
                <GiOilDrum className="text-purple-700 text-lg sm:text-xl 
                                      flex-shrink-0 mt-1" />
                <div>
