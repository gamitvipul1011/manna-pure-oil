...
const ProductDetail = () => {
  ...
  useEffect(() => {
    ...
    let relatedProducts = products.filter(
      (x) => String(x._id) !== String(id) && x.category._id === p.category._id
    );

    if (relatedProducts.length === 0) {
      relatedProducts = products.filter((x) => String(x._id) !== String(id));
    }
  ...
  // ✅ Fix 2: handleAddToCart returns true/false
  const handleAddToCart = () => {
    if (!selectedSize) {
      toast.error(isGu ? "કૃપા કરી સાઈઝ પસંદ કરો" : "Please select size");
      return false;
    }
    addToCart(
  ...
  // ✅ Fix 3: Correct regex for parseLines
          ...
          <div className="space-y-4">
            ...
            <div className="relative flex justify-center items-center">
              ...
              <div className="relative inline-flex justify-center items-center overflow-hidden rounded-[28px]">
                ...
                {/* ✅ Fix 5: key={displayImage} so loader resets on image change */}
                <img
                  key={displayImage}
                  src={displayImage}
                  alt={product.name}
                ...
                {currentImages.map((img, idx) => (
                  <button
                    ...
                    className={`w-14 h-14 sm:w-16 sm:h-16 md:w-20 md:h-20 flex-shrink-0 
                      overflow-hidden border-2 transition-all duration-200 rounded-xl ${
          ...
          <div className="space-y-5 sm:space-y-6">
                ...
                <div className="flex flex-wrap gap-2 sm:gap-3">
                  {product.sizes.map((sv, idx) => (
                    <button
                      ...
                      className={`px-4 sm:px-5 py-2 rounded-lg sm:rounded-xl border 
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl 
                  text-white font-semibold transition-all duration-300 text-sm sm:text-base ${
                    addedAnim
              ...
              {/* ✅ Fix 6: Buy Now validate before navigate */}
              <button
                onClick={() => {
                ...
                className="flex-1 py-3 rounded-xl bg-orange-500 hover:bg-orange-600 
                  text-white font-semibold transition-all active:scale-95 text-sm sm:text-base"
              >
            ...
            <a
              ...
              className="flex items-center justify-center gap-3 w-full py-3 rounded-xl 
                text-white bg-green-500 hover:bg-green-600 font-semibold transition-all 
          ...
          <div className="flex gap-1 sm:gap-2 border-b border-purple-400/30 overflow-x-auto">
            {tabs.map((tab) => (
              <button
                ...
                className={`px-4 sm:px-6 py-2.5 sm:py-3 font-bold text-sm sm:text-base 
                  rounded-t-xl transition-all whitespace-nowrap ${
                    activeTab === tab.key
