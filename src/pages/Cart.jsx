...
const Cart = () => {
  ...
  // Product weight find karva mate helper
  const getItemWeightInKg = (item) => {
    // 1) Direct kg field hoy to
    if (item.weightInKg !== undefined && item.weightInKg !== null) {
      return Number(item.weightInKg) || 0;
    }

    // 2) weight field hoy ane kg ma hoy to
    if (item.weight !== undefined && item.weight !== null) {
      return Number(item.weight) || 0;
    }

    // 3) grams field hoy to kg ma convert
    if (item.weightInGrams !== undefined && item.weightInGrams !== null) {
      return (Number(item.weightInGrams) || 0) / 1000;
    }

    // 4) size mathi parse karvu, ex: "500g", "1kg"
    const sizeText = String(item.size || '').toLowerCase().trim();

    const kgMatch = sizeText.match(/(\d+(\.\d+)?)\s*kg/);
    if (kgMatch) {
      return parseFloat(kgMatch[1]) || 0;
    }

    const gramMatch = sizeText.match(/(\d+(\.\d+)?)\s*(g|gm|grams?)/);
    if (gramMatch) {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    {itemWeight > 0 && (
                      <p className="text-sm text-gray-500 mb-1">
                        Weight: {itemWeight} kg × {item.quantity}
                      </p>
                    )}
                    <p className="text-3xl font-bold text-brand-orange-600">₹{Number(item.price).toFixed(2)}</p>
                  </div>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-3xl font-bold text-brand-purple-800">₹{itemTotal.toFixed(2)}</p>
                  </div>
                </div>
              );
            })}
              ...
              <div className="space-y-4 mb-6">
                ...
                <div className="border-t-2 border-gray-300 pt-4">
                  <div className="flex justify-between text-2xl font-bold">
                    ...
                    <span className="text-brand-orange-600">₹{finalTotal.toFixed(2)}</span>
                  </div>
                </div>
              </div>
