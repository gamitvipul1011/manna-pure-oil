...
const FAQ = () => {
  ...
  return (
    <div className="min-h-screen bg-[#A855F7] py-16">
      <div className="max-w-3xl mx-auto px-6">
        ...
        <div className="space-y-4">
          {faqs.map((faq, i) => (
            <div key={i} className="bg-[#D0F0C0] rounded-2xl shadow-md overflow-hidden hover:shadow-lg transition">
              <button
                className="w-full flex items-center justify-between p-6 text-left"
                onClick={() => setOpen(open === i ? null : i)}
              >
                <span className="font-bold text-green-800 pr-4">{faq.q}</span>
                {open === i
