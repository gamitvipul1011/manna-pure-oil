...
import bannerImage from '../assets/about1.jpeg';
...
const Contact = () => {
  ...
  const contactInfo = [
    { icon: <FaPhone />, en: 'Phone ', gu: 'મોબાઈલ નંબર', value: '+91 78742 39595', color: 'bg-green-50 text-green-600' },
    { icon: <FaEnvelope />, en: 'Email', gu: 'ઈમેલ', value: 'maanapureoil@gmail.com', color: 'bg-blue-50 text-blue-600' },
    { icon: <FaMapMarkerAlt />, en: 'Address', gu: 'સરનામું', value: 'Dolara, Vyara-Ahwa Rd, Tapi, Gujarat', color: 'bg-orange-50 text-orange-600' },
  ];
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="relative w-full h-[350px] md:h-[420px] overflow-hidden">
        <img src={bannerImage} alt="Contact" className="w-full h-full object-cover scale-105" />
        <div className="absolute inset-0 bg-gradient-to-b from-green-900/70 to-green-800/50"></div>
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center text-white px-6">
          <h1 className="text-5xl md:text-6xl font-extrabold mb-3 drop-shadow-lg">
            {isGu ? 'સંપર્ક કરો' : t('contactTitle')}
          </h1>
          <p className="text-xl text-green-100 mb-2">{t('contactSubtitle')}</p>
          <p className="text-green-200 text-base">Pure Cold Pressed Oil – 100% Natural</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-16 grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8 border border-gray-100">
          <h2 className="text-3xl font-extrabold text-green-700 mb-1">
            {isGu ? 'sandesh moklo' : 'Send a Message'}
          </h2>
          <p className="text-gray-400 text-sm mb-7">
            {isGu ? 'ame 24 kalaak maa javab aapisu' : 'We reply within 24 hours'}
          </p>
          ...
          {sent ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <h3 className="text-2xl font-bold text-green-700 mb-2">
                {isGu ? 'sandesh mokalyO!' : 'Message Sent!'}
              </h3>
              <p className="text-gray-500">
                {isGu ? 'ame jaldi javab aapisu' : "We'll get back to you soon"}
              </p>
            </div>
          ) : (
        ...
        <div className="space-y-5">
          {contactInfo.map((info, i) => (
            <div key={i} className="flex items-center gap-5 bg-white rounded-2xl shadow-lg p-6 hover:shadow-xl transition-shadow border border-gray-50">
              <div className={`p-4 rounded-2xl ${info.color} text-2xl`}>{info.icon}</div>
              <div>
